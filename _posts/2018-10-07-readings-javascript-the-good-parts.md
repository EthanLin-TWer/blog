...
本书中关于 JavaScript 的精华与糟粕，说白了最后就是一个 ESLint 规则，强制必须使用哪些好的写法，哪些坏的写法一定不让用。关于编程风格方面的东西，现在已经有 prettier 这样的工具，提供一个唯一的格式化方案，可以杜绝团队内部的风格争论，提高工作效率和代码整体统一性。经过试用，它的这个唯一的格式化方案还相当完美，我可以毫不夸张的说，在我工作过的项目中，99.99%的场景使用 prettier 格式化出来的风格都是完美的。
