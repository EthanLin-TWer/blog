...
1. 为什么要写这篇文章
   1. 看程墨的《深入浅出》，感觉单元测试那一章写得有些入门，对单元测试的定调不正确，也没有回答「为什么要做单元测试」这个问题
   2. 从后端来，深受 Clean-Code 和 TDD-Refactoring 感染；入前端后，发现前端群体都专注在怎么写出代码上，对单元测试这一块重视度较后端低（怎么得出这个结论？凭感觉？），从而导致写出来的代码不具可测性，往往也不符合基本的 SRP 设计原则，较难维护和读懂，更无法重构，因为没有测试
2. 为什么要做单元测试
   1. 谈单元测试的上下文是什么？是敏捷，是又快又要质量好的需求
   2. 质量好就只能写通过写测试达到吗？当然不是，还可以通过架构（还有其他方式吗）
   3. 写测试就只能写单元测试吗？当然也不是，但单元测试是主力。依据为何？测试金字塔
3. 如何做单元测试
   1. 首先，什么是好的单元测试？它有什么特征？
...
从客观性的角度讲，程墨在[《深入浅出 React & Redux》](https://book.douban.com/subject/27033213/)那章单元测试讲的比我客观；从内容的角度讲，我觉得讲得没有我这节 issue 好。它没有抓住重点，并且只是站在测试的角度来谈测试，只介绍了如何写单元测试，对为什么要写单元测试并没有分析透彻。由于它客观，导致它没有一个鲜明的观点。

作者认为，不争论 TDD 是否有必要（当然不争论，肯定有必要），最重要一点是：单元测试应该是让开发者的工作更轻松更高效，而不是成为开发过程中的包袱。不晓得言下之意，是不是说，如果（因为不会写，写不好而导致）单元测试成为了开发过程中的包袱，那就可以果断不要单元测试。

这一点我的观点是鲜明的：单元测试是一定要的。React 的单元测试客观上是可以让开发者的工作更高效的，写不好是能力问题，不写则是态度问题。
