...
由于 JS 中函数是一等公民，所以函数名这个东西，运行时是能拿到的，但是函数参数就不行了；又由于 JS 中有太过灵活的 `eval()` 函数，所以根据字符串名称生成对应类实例的事，变着法子也是能做的 `eval(\`new ${className.toUpperCase()}()\`)`，但反射 API 中是没有这部分的。这一切，跟编译、类型系统有密切的关系。我的结论是，**JavaScript 反射在获取程序元信息上的能力是比较弱的**。
