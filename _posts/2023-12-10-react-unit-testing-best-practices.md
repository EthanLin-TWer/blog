...
> 
> 使用RTL（React testing library）基于JSDom的测试，我们可以把这种测试的运行速度仍然限制在单元测试的范畴中。
...
同时，为了达到支撑重构的根本目标，我们也不可避免地在一般的“单元测试”上引入了许多元素和分层，这些要素也必然会带来新的复杂度：

* **（一次性投入的）开发成本**。从零开始应用这套测试策略需要编写不少的代码（mock API、编写component tester等等），需要进行一些选型和设计，还可能需要踩一些坑（未在本文体现😈欢迎电询作者）以让这套工具更加稳定、通用。这是需要预期及被考虑的。
...
即便有这些挑战，我们仍然提倡通过这种方式来编写单元测试，是因为看重它**能有效支撑重构**的好处，而这一点在我经历过的前端项目和测试中尤其重要。这一点在本文中已经强调过很多次，希望读者在取用时能理解这个出发点及其取舍。
