...
当我明白组件可以仅存在于源码单元上的时候，我自然就有了更多的疑问：#1 那有什么标准判断哪些东西应该被划到同一个组件里呢（我非说就一个`index.js`它也是一个组件存在独立部署的潜力，按照定义也没毛病啊？那如果任意文件集合都可以是组件，组件这定义不就等于变成口头圈地白说了么？我一个应用有6万个组件）？#2 源码意义上的组件跟代码“模块”有什么区别呢？#3 源码意义上的组件没有发生部署动作，不存在部署等各方面成本，那么源码层次的组件划分有什么意义和作用呢？

这第一个问题，其实就引出了如何划分组件的话题。诚然，组件的边界可以划在任何地方，但是划的好坏有差异。第13章介绍的就是组件聚合的原则，指出符合原则的组件拆分可以得到什么好处；第14章介绍组件设计中的耦合现象，指出不符合原则的组件拆分将会带来什么坏处。
...
CRP原则指的是，不要强迫用户依赖他们不需要的东西。一个包C发出去A和B在用，但是A只关心C1功能，B只关心C2功能，这不好，用户可能经常收到他不需要的更新通知（好在用户有选择不更新的自由，但也是白费了阅读changelog的成本）。但是这个麻烦总归在用户，所以我不太理解作者在13章中说违反CRP原则会导致“太多不必要的发布”是什么意思？对于组件的作者来说，他发布C或单独发布C1都是同样的成本，次数并没有变多；对于组件的用户来说，我可以选择不升级，也不存在“太多不必要的发布”。

所以读13章，我的建议是，忽略组件张力图。具体实践时，我们一般会站住REP原则（也就是通过版本化的方式发布可复用包），在CCP和CRP中取舍，而且看起来CCP更加重要。违反CRP，最多就是用户多收到些无关的更新通知；违反CCP，组件作者要付出“过多组件变更和发布”的成本，组件用户要付出“同时更新”的成本。
