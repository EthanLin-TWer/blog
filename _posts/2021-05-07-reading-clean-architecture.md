...
16章指出，这样的隔离可以在用例和分层两方面展开，以实现层间、用例间互相独立。按层解耦，一个系统一般可以被解耦成若干个水平分层：UI界面、业务逻辑、数据库；按用例解耦，则是垂直方向的解耦，每个用例都可以跨越若干分层。注意，解耦可以发生在不同层面，也未说明层间实际的依赖关系。

19章指出，低层组件（不稳定的、具体的细节）应被设计为依赖于高层组件（稳定的、抽象的策略）。也就是说，分层中的UI和数据库都应该依赖于业务逻辑，因为前两者不属于核心部分，不稳定。
